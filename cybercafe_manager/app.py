...
def init_db():
    ...
    # Optimization (Bolt ⚡): Enable Write-Ahead Logging (WAL) & normal synchronous mode
    # WAL allows concurrent reads during writes and reduces disk I/O latency on Android storage.
    ...
    # Optimization (Bolt ⚡): Add indexes on frequently queried/joined fields
    # Speed up financial aggregation (transactions.created_at)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_transactions_created_at ON transactions(created_at);")
    # Speed up session state lookups and terminal joins
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_sessions_terminal_start ON sessions(terminal_id, start_time);")
    # Speed up connection log lookup during logout
