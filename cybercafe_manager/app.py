...
def init_db():
    ...
    # --- INDEX DE PERFORMANCE POUR BASE DE DONNÉES ---
    # Performance optimization: Add indexes for frequently queried columns in background/dashboard polling endpoints
    # - transactions(created_at): Accelerates get_financial_summary() and revenue reports filtered by date
    # - connection_logs(terminal_name, logout_time): Accelerates session logout lookups
    # - sessions(terminal_id): Accelerates JOIN and filtering operations on active sessions per terminal
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_sessions_terminal_id ON sessions(terminal_id);")
...
def admin_reports_print():
    ...
    cursor.execute('''
        SELECT terminal_name as name, COUNT(*) as count, SUM(duration_mins * 60) as total_seconds
