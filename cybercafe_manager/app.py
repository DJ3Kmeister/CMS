...
def admin_reports_print():
    ...
    cursor.execute("SELECT COUNT(*) as count FROM sessions WHERE start_time >= ?", (start_date_str,))
    total_sessions = cursor.fetchone()['count']

    cursor.execute("""
        SELECT terminal_id, COUNT(*) as session_count, SUM(duration_mins) as total_duration
        FROM sessions
        WHERE start_time >= ?
